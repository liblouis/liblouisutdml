...
/*Examine the parse tree, add semantic attributes and set indicators.*/
  xmlNode *child;
  lbu_logMessage(LOG_INFO, "Begin examine_document: node->name=%s", node->name);
  if (node == NULL)
    return 0;
...
{
  lbu_logMessage(LOG_INFO, "Begin examText: node->content=%s", node->content);
  switch (ud->stack[ud->top])
    {
    case pagenum:
      break;
...
{
  lbu_logMessage(LOG_INFO, "Begin examCdata");
  ud->has_cdata = 1;
}
