...
int finish_endnote(xmlNode* node)
{
	...
	{
		lbu_logMessage(LOG_DEBUG,"finish_endnote:Endnote with id %s already exists",get_attr_value(node));
		return 0;
	}
...
int link_endnote(xmlNode* node)
{
	...
	{
		lbu_logMessage(LOG_DEBUG,"link_endnote:Endnote with id %s already exists",get_attr_value(node));
		return 0;
	}
