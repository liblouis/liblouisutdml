...
brf_findBrlNodes (xmlNode * node)
{
  ...
  switch (ud->stack[ud->top])
    ...
    case utdmeta:
      return 1;
    case utdbrl:
      lbu_logMessage(LOG_DEBUG, "Processing brl node");
...
{
  lbu_logMessage(LOG_DEBUG, "brf_doDotsText %s", node->content);
  ud->text_length = 0;
  insert_utf8 (node->content);
  if ((ud->outbuf1_len_so_far + ud->text_length) > ud->outbuf1_len)
    brf_saveBuffer ();
  lbu_logMessage(LOG_DEBUG, "text_buffer: %s", ud->text_buffer);
  if (!lou_dotsToChar (ud->main_braille_table, ud->text_buffer,
		       &ud->outbuf1[ud->outbuf1_len_so_far],
		       ud->text_length, 0))
    return 0;
  lbu_logMessage(LOG_DEBUG, "ud->textLength %d", ud->text_length);
  lbu_logMessage(LOG_DEBUG, "ud->outbuf1: %s", &ud->outbuf1[ud->outbuf1_len_so_far+1]);
