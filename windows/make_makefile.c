...
"!include $(LIBLOUIS_SRC_PATH)\\..\\windows\\configure.mk", 
...
if ((makefile_am =
	  fopen ("..\\liblouisutdml\\Makefile.am", "r")) == NULL)
...
if ((Makefile = fopen ("Makefile.msvc", "w")) == NULL)
...
if (!(name[nameLength - 2] == '.' && name[nameLength - 1] == 'c'))
	
 
continue;
      
 
strncpy (module[moduleCount++], name, nameLength - 2);
...
fprintf (Makefile, "%s.obj: builddir $(HEADERS) %s.c\n",
		  
module[kk], module[kk]);
      
 
fprintf (Makefile, "    $(CC) $(CCFLAGS) %s.c\n", module[kk]);
    
 
}
  
 
fprintf (Makefile, "builddir: builddir\n");
  
 
fprintf (Makefile, "    if not exists builddir mkdir builddir\n");
